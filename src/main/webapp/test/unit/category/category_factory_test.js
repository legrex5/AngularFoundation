...
describe('services', function(){
  ...
  describe('Category', function(){
    ...
    it('create throws exception : id not defined', function() {
    	try{
    		Category.create({categoryid:null,name:'category'});
    		$httpBackend.flush();
    	} catch(errors) {
    ...
    it('create', function() {
    	...
    	Category.create({categoryid:'1',name:'category'}).then(function(response) {
        	expect(response.status).toBe(200);
    		expect(response.data).toBe("test");
    	});
    	$httpBackend.flush();
    ...
    it('update throws exception : id not defined', function() {
    	try{
    		Category.update({categoryid:null,name:'category'});
    		$httpBackend.flush();
    	} catch(errors) {
    		expect(errors[0]).toBe('category.id.not.defined');
    	}
    ...
    it('update', function() {
    	...
    	Category.update({categoryid:'1',name:'category'}).then(function(response) {
        	expect(response.status).toBe(200);
    		expect(response.data).toBe("test");
    	});
    	$httpBackend.flush();
