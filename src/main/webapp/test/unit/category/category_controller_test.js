...
describe('controllers', function(){
  ...
  describe('CategoryCtrl', function(){
    var CategoryCtrl, Category, Useraccess, $rootScope, $scope, $routeParams, $httpBackend, $location, MessageHandler, $q, $controller;
    ...
    it('refreshCategory', function() {
    	...
    	Category.get = function(categoryid) {
			var deferred = $q.defer();
			deferred.resolve({data:'category'+categoryid});
			return deferred.promise;
		}
    ...
    it('goToCategory', function() {
    	...
    	// when
    	$scope.goToCategory(categoryid);
    	$scope.$apply();
    	
    	// then
    	expect($scope.refreshCategory).toHaveBeenCalledWith(categoryid);
    	expect($location.path).toHaveBeenCalledWith('/category'+'/'+categoryid);
    });
    
    it('save : create', function() {
    	// given
    	$scope.category = {categoryid:'1', name:'category'};
